...
function FormulaireEtudiant({ etudiant, filieres, onAnnuler, onEnregistrer }) {
  const [matricule, setMatricule] = useState(etudiant?.matricule ?? '');
  ...
  async function gererEnvoi(e) {
    ...
    setEnCours(true);
    try {
      await onEnregistrer({ matricule, nom, prenom, filiere_id: filiereId, niveau, annee_academique: anneeAcademique });
    } catch (err) {
